...
global_layers = VersionAwareLayers()
...
def MobileNetV3Large(input_shape=None,
    ...
    def stack_fn(x, kernel, activation, se_ratio):
        ...
        x = _inverted_res_block(x, 1, depth(16), 3, 1, None, relu, 0, l)
        x = _inverted_res_block(x, 4, depth(24), 3, 2, None, relu, 1, l)
        x = _inverted_res_block(x, 3, depth(24), 3, 1, None, relu, 2, l)
        x = _inverted_res_block(x, 3, depth(40), kernel, 2, se_ratio, relu, 3, l)
        x = _inverted_res_block(x, 3, depth(40), kernel, 1, se_ratio, relu, 4, l)
        x = _inverted_res_block(x, 3, depth(40), kernel, 1, se_ratio, relu, 5, l)
        x = _inverted_res_block(x, 6, depth(80), 3, 2, None, activation, 6, l)
        x = _inverted_res_block(x, 2.5, depth(80), 3, 1, None, activation, 7, l)
        x = _inverted_res_block(x, 2.3, depth(80), 3, 1, None, activation, 8, l)
        x = _inverted_res_block(x, 2.3, depth(80), 3, 1, None, activation, 9, l)
        x = _inverted_res_block(x, 6, depth(112), 3, 1, se_ratio, activation, 10, l)
        x = _inverted_res_block(x, 6, depth(112), 3, 1, se_ratio, activation, 11, l)
        x = _inverted_res_block(x, 6, depth(160), kernel, 2, se_ratio, activation,
                                12, l, bottomright_stride=centered_stride)
...
def relu(x):
    return global_layers.ReLU()(x)


def hard_sigmoid(x):
    return global_layers.ReLU(6.)(x + 3.) * (1. / 6.)


def hard_swish(x):
    return global_layers.Multiply()([hard_sigmoid(x), x])
...
def _se_block(inputs, filters, se_ratio, prefix):
    ...
    x = global_layers.Conv2D(
        ...
        x)
    x = global_layers.ReLU(name=prefix + 'squeeze_excite/Relu')(x)
    x = global_layers.Conv2D(
        filters,
        kernel_size=1,
        padding='same',
        ...
        x)
    x = hard_sigmoid(x)
    x = global_layers.Multiply(name=prefix + 'squeeze_excite/Mul')([inputs, x])
    return x


def _inverted_res_block(x, expansion, filters, kernel_size, stride, se_ratio,
                        activation, block_id, layers, bottomright_stride=False):
    channel_axis = 1 if backend.image_data_format() == 'channels_first' else -1
    shortcut = x
    ...
    if stride == 2:
        shift = 1 if bottomright_stride else 0
        x = layers.ZeroPadding2D(
            padding=correct_pad(kernel_size, shift),
...
def correct_pad(kernel_size, shift):
    ...
    return ((pad_beg - shift, pad_end + shift), (pad_beg - shift, pad_end + shift))
